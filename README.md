...
---
## 题目总表V2（30 题，补充难度评估）
| 优先级        | 题号  | 题目                               |  理解⭐  |  编码⭐  |  记忆⭐ | 总耗时       | 学习策略                                          |
| :--------- | :-- | :------------------------------- | :---: | :---: | :--: | :-------- | :-------------------------------------------- |
| **Tier S** |     |                                  |       |       |      |           |                                               |
| S1         | Q02 | Causal Self-Attention + past\_kv |  ⭐⭐⭐  | ⭐⭐⭐⭐⭐ |  ⭐⭐⭐ | **3h**    | 先纸上演算 shape，再写代码；必须背熟 Q/K/V reshape 和 mask 逻辑 |
| S2         | Q01 | KV Cache 管理器（GQA 共享）             |   ⭐⭐  |  ⭐⭐⭐⭐ |  ⭐⭐  | **1.5h**  | 直接写代码调试；重点练 Python 引用陷阱（深拷贝）                  |
| S3         | Q06 | ReAct Agent 状态机                  |  ⭐⭐⭐  |  ⭐⭐⭐⭐ |  ⭐⭐  | **2h**    | 先画状态转换图（纸笔 10min），再转代码；if/elif 链要清晰           |
| S4         | Q08 | 动作 Tokenization（连续→离散 bin）       |   ⭐⭐  |   ⭐⭐  |   ⭐  | **1h**    | 纯数学题，直接写；clip + bin 公式背下来                     |
| **Tier A** |     |                                  |       |       |      |           |                                               |
| A5         | Q03 | RoPE 位置编码                        | ⭐⭐⭐⭐⭐ |  ⭐⭐⭐  | ⭐⭐⭐⭐ | **3h+**   | **必须先手推公式**（旋转矩阵），再编码；缓存 sin/cos 是记忆重点        |
| A6         | Q04 | Top-K / Top-P 采样                 |  ⭐⭐⭐  |  ⭐⭐⭐  |  ⭐⭐  | **1.5h**  | 概率逻辑容易绕，建议先写 Greedy，再叠加 TopK，最后 TopP          |
| A7         | Q07 | Function Calling 解析器             |   ⭐⭐  |  ⭐⭐⭐⭐ |   ⭐  | **1.5h**  | 直接写；JSON 解析 + 递归 dict 遍历，边写边测                 |
| A8         | Q10 | PagedAttention BlockManager      |  ⭐⭐⭐  |  ⭐⭐⭐⭐ |  ⭐⭐  | **2h**    | 内存池思想；set/dict 操作是 Python 特有，需熟悉              |
| A9         | P09 | Attention + past\_kv（GQA 综合版）    |  ⭐⭐⭐  |  ⭐⭐⭐⭐ |  ⭐⭐⭐ | **1h**    | 若 Q02/Q05 已会，此题只需**读题确认思路**，不手写完整代码           |
| A10        | P02 | KV Cache GQA（内存计算版）              |   ⭐⭐  |   ⭐⭐  |  ⭐⭐  | **30min** | 纯公式套算；背熟 `2*B*S*L*H*D*P/10^9`                 |
| A11        | P03 | 动作 Tokenizer（7DoF 综合版）           |   ⭐⭐  |   ⭐⭐  |   ⭐  | **30min** | Q08 熟练后，此题只练速度                                |
| A12        | Q05 | MQA（Multi-Query Attention）       |  ⭐⭐⭐  |  ⭐⭐⭐  |  ⭐⭐  | **1.5h**  | 重点理解"广播"概念；编码比 MHA 简单（参数少）                    |
| **Tier B** |     |                                  |       |       |      |           |                                               |
| B13        | Q15 | FlashAttention（分块 Softmax）       | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | **4h+**   | **最难一题**；建议先看参考实现理解 online softmax，再闭卷写       |
| B14        | Q12 | INT8 对称量化/反量化                    |  ⭐⭐⭐  |   ⭐⭐  |  ⭐⭐  | **1h**    | 公式题；scale/round/clip 三步背熟                     |
| B15        | Q11 | Continuous Batching 调度器          |  ⭐⭐⭐  |  ⭐⭐⭐⭐ |   ⭐  | **2h**    | 调度逻辑易错；建议画时序图再写代码                             |
| B16        | P05 | Top-K + Top-P（Sampler 类）         |  ⭐⭐⭐  |  ⭐⭐⭐  |  ⭐⭐  | **45min** | Q04 的 OO 封装版；会 Q04 则快速过                       |
| B17        | Q17 | Agent 记忆系统（短期+长期）                |   ⭐⭐  |  ⭐⭐⭐  |   ⭐  | **1.5h**  | 余弦相似度公式背一下；deque + list 组合                    |
| B18        | Q18 | 任务 FSM（超时/回退/重试）                 |  ⭐⭐⭐  |  ⭐⭐⭐⭐ |  ⭐⭐  | **2h**    | 状态机 + 超时检测；time.time() 用法需熟悉                  |
| B19        | P10 | 量化内存计算器                          |   ⭐⭐  |   ⭐   |  ⭐⭐  | **30min** | 纯计算函数；背公式即可                                   |
| B20        | P06 | BlockManager（引用计数版）              |  ⭐⭐⭐  |  ⭐⭐⭐  |   ⭐  | **45min** | Q10 基础上加 ref\_count；逻辑简单但易漏边界                 |
| **Tier C** |     |                                  |       |       |      |           |                                               |
| C21        | Q13 | Ring AllReduce 模拟                |  ⭐⭐⭐  |   ⭐⭐  |   ⭐  | **1h**    | 通信逻辑抽象；编码只是列表操作                               |
| C22        | Q14 | 流水线气泡计算函数                        |   ⭐   |   ⭐   |   ⭐  | **30min** | 纯公式题；背 bubble\_rate 公式                        |
| C23        | Q16 | AMP 混合精度状态管理                     |  ⭐⭐⭐⭐ |  ⭐⭐⭐⭐ |  ⭐⭐⭐ | **2.5h**  | 概念多（FP16/FP32/loss scale）；编码复杂                |
| C24        | Q19 | 优先级调度器（急停插队）                     |  ⭐⭐⭐  | ⭐⭐⭐⭐⭐ |   ⭐  | **2.5h**  | 调度逻辑复杂；踢出/插队边界极易错                             |
| C25        | Q20 | VLA 数据闭环流程模拟                     |   ⭐⭐  |  ⭐⭐⭐  |   ⭐  | **1.5h**  | 流程长但每步简单；filter/augment/tokenize 串联           |
| C26        | Q09 | 简化 VLA Forward                   |  ⭐⭐⭐  |  ⭐⭐⭐  |  ⭐⭐  | **1.5h**  | 多模态拼接思想；编码量中等                                 |
| C27        | P01 | LRU Cache（LeetCode 146）          |   ⭐⭐  |  ⭐⭐⭐  |   ⭐  | **1h**    | 传统算法题；自变量笔试**几乎不考**                           |
| C28        | P04 | Tool Parser（嵌套 JSON 版）           |   ⭐⭐  |  ⭐⭐⭐  |   ⭐  | **45min** | Q07 熟练后快速过                                    |
| C29        | P07 | ReAct Step（带工具注册）                |   ⭐⭐  |  ⭐⭐⭐  |   ⭐  | **45min** | Q06 熟练后快速过                                    |
| C30        | P08 | RoPE 缓存动态扩展版                     |  ⭐⭐⭐⭐ |  ⭐⭐⭐  |  ⭐⭐⭐ | **1h**    | Q03 熟练后快速过                                    |
---
